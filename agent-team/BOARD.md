...
1. Package the new Linux `1.3.0` archive only if high-performance evidence
   meets the Windows-equivalent gate; then rehash and smoke-test it.
2. Run the protected Linux `1.1.1` package under the same Ubuntu host and
   report it as a historical-version result, not as a replacement artifact.

## Validation still required

- Linux package integrity and 1.1.1 comparative checks remain required.
  Windows acceptance remains complete.
