...
- 2026-07-19 已修复高性能模式仍创建前端主窗口并无条件拉起靶场 debug 子进程的问题：`DAEDALUS_PERF_DISABLE_UI=1` 时 `WindowPlugin.primary_window=None`，debug 子进程管理系统不运行；`-Visible` 行为保持不变。对应单元测试通过，尚未打包新 Release。
