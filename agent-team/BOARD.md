...
- 模拟器 `1.0.3` 已从清洁提交 `c245836` 正式发布：1 ms 无窗口调度节拍修复 1.0.2 的 readback/TCP 积压。完整自瞄静止靶复验恢复 exact-exposure 与约 64 Hz 视觉；独立 200 Hz 越权请求仍被模拟器钳制为 19.2308 Hz，桌面窗口句柄为 0。SDK 保持 `1.0.0`。
