...
- 代码基线：本文件所在提交
- 模拟器版本：`Daedalus Simulator 1.0.2`
...
当前完成独立仓库拆分：Rust 生产端、公共 C++ SDK、版本契约和 Release 工具均由本仓库单点发布。自瞄 B 是首个消费者；打符适配状态由消费者仓库维护。`1.0.2` 固化高性能模式无主窗口/无 debug 子进程，并在模拟器内部强制 20 Hz 最大物理射频；SDK 与 IPC 契约不变。
