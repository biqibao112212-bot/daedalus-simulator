...
8 秒、连续采样 20 秒”的旧口径补测。

### 11.2 Linux RTX 3090 完整包验收（2026-07-30）

模拟器 1.1.1 Linux x86_64 RC2 在 AutoDL Ubuntu 22.04、Xeon Gold 6330、RTX 3090、
NVIDIA 580.76.05、Vulkan 无显示模式下，用发布包内 C++ SDK 完成 20.007 秒采样：

| 指标 | 实测值 |
| --- | ---: |
| SDK 实收图像 | 723 帧，36.138 FPS |
| 单帧 payload | 6,220,800 B（1440×1080 RGBA32） |
| payload 吞吐 | 214.393 MiB/s |
| 同帧云台/时间戳 | 722 次成功，成功项全部严格匹配 |
| 云台命令反馈 | `command_id=1` 已在最新状态中确认应用 |
| 场景控制 | `ping/create_session/status` 全部 ACK |
| TCP 超时 | 0 |
| GPU | 平均 13.185%，最大 24%，显存最大 947 MiB |

该服务器的模拟器进程约占 329% CPU，而 GPU 平均利用率只有 13.185%，所以 36 FPS 是该
Xeon/Vulkan 环境的 CPU 受限基线，不是 RTX 3090 上限。它与 Windows 的 i9/DX12/并发
训练短测不是同机同条件对照，不能用差值判断系统优劣。结论是 Linux 完整包已经能在无
桌面的真实 NVIDIA GPU 服务器上稳定完成图像、时间戳、同帧云台、控制反馈和场景控制
闭环；llvmpipe 仍只用于无独显启动诊断。
