...
### Linux RTX 3090 完整闭环验收（2026-07-30）

- AutoDL Ubuntu 22.04 / Xeon Gold 6330 / RTX 3090 / NVIDIA 580.76.05 上，从 GitHub
  RC2 Release 经官方加速通道下载完整 TAR.GZ，服务端 SHA256 与本地一致。
- 无 `DISPLAY`、Wayland 或 Xvfb；使用 AutoDL 官方建议的 `libEGL_nvidia.so.0` 无显示
  Vulkan ICD，运行时能力确认实际适配器为 RTX 3090，而不是 llvmpipe。
- 包内非 root 安装器通过，安装后二进制与包内二进制逐字节一致；发布 SDK 在服务器上由
  GNU C++17 工具链独立编译链接。
- 预热 3 秒、连续采样 20.007 秒，取得 723 帧 1440×1080 RGBA32：36.138 FPS、
  214.393 MiB/s、TCP 超时 0。
- 722 次成功的同帧云台查询均与图像曝光时间戳严格一致；最新状态确认云台命令 ID 已应用，
  `ping/create_session/status` 场景控制全部 ACK。
- GPU 利用率平均 13.185%、最大 24%，模拟器约占 329% CPU，因此这是该 Xeon/Vulkan
  环境的 CPU 受限基线，不是 RTX 3090 上限，也不与 Windows/i9/DX12 数值直接横比。
- 验收过程中发现并修复 Linux 无窗口模式的固定步长追赶循环，以及低帧率下云台接收与
  应用跨帧导致的过期问题。
...
Windows 完成完整图像与控制闭环验收；Linux 同时完成 llvmpipe 无独显兼容诊断和 RTX
3090 无显示硬件 Vulkan 的完整图像、时间戳、云台与场景控制闭环验收。当前包仅用于所属
实验室内部非商业培训和研究；若要提供给其他组织或公开发布，必须另行审查许可证和第三方
依赖。
